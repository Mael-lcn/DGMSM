...
def get_shared_parser():
    ...
    group_model.add_argument('--in_channels', type=int, default=3, 
                             help="Nombre de canaux en entrée (RGB)")
    group_model.add_argument('--out_channels', type=int, default=3, 
                             help="Nombre de canaux en sortie (Reconstruction RGB)")

    # nargs='+' permet de passer une liste via le terminal
    group_model.add_argument('--hidden_dims', type=int, nargs='+', default=[64, 128, 256, 256, 128], 
                             help="Liste des dimensions cachées de l'encodeur/décodeur")
    group_model.add_argument('--kernel_size', type=int, default=3, 
                             help="Taille du kernel convolutif")


    group_model.add_argument('--padding_mode', type=str, default="same", choices=["same", "valid"], 
                             help="Mode de padding ('zeros' garde la taille de l'image intacte)")
    group_model.add_argument('--skip_mode', type=str, default="none", choices=["concat", "add", "none"], 
                             help="Mode des skip-connections. (Alerte : mettre 'none' pour un vrai Autoencodeur)")
    group_model.add_argument('--upsampling_mode', type=str, default="transpose", choices=["transpose", "bilinear"], 
