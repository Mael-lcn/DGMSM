...
from .sampling_util import sampling_major_vote_func, CMR_sampling_major_vote_func
...
class TrainLoop:
    ...
    def run_loop(self, max_iter=250000, start_print_iter=100000, vis_batch_size=8, n_rounds=3):
        ...
        while (
            ...
            batch = data_["GT"]
            # print(batch.shape)
            cond = {"conditioned_image": data_["input"]}
            # batch, cond = next(self.data)[1]["input"], next(self.data)[1]["GT"]
            ...
            if self.step % self.save_interval == 0 and self.step >= start_print_iter or self.step == 60000:
                ...
                set_random_seed_for_iterations(self.step)
                dist.barrier()
            self.step += 1
    
    def run_testcheck(self, max_iter=250000, start_print_iter=100000, vis_batch_size=8, n_rounds=3):
        if dist.get_rank() == 0:
            pbar = tqdm()

        logger.log(f"manually check test")
        self.ddp_model.eval()
        ...
        self.ema_model.load_state_dict(self._master_params_to_state_dict(self.ema_params[0]))
        self.ema_model.eval()
        ema_val_psnr, ema_val_ssim = CMR_sampling_major_vote_func(self.diffusion, self.ema_model, output_folder=output_folder,
